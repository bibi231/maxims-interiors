...
const STATUS_STYLE = {
  ...
  rescheduled:  'text-cream-soft bg-cream-soft/5 border-cream-soft/10',
}
    ...
    <AdminLayout>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="font-body text-[0.75rem] text-cream-soft mt-0.5">{appts.length} total</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {Object.entries(grouped).sort(([a],[b]) => a.localeCompare(b)).map(([date, dayAppts]) => (
            <div key={date}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="font-title text-[0.55rem] tracking-wider text-cream-soft">{dayAppts.length} appt{dayAppts.length !== 1 ? 's':''}</div>
              </div>
                  ...
                  <motion.div key={appt.id}
                    ...
                    <div className="flex items-center gap-4 px-5 py-4">
                      <div className="text-center shrink-0 w-14">
                        ...
                        <div className="font-body text-[0.58rem] text-cream-soft mt-0.5">{appt.duration_mins}min</div>
                      </div>
                      <div className="w-px h-10 bg-gold/12 shrink-0" />
                      <div className="flex-1 min-w-0">
                        <div className="font-body text-[0.88rem] text-cream-soft">{appt.client_name}</div>
                        <div className="flex items-center gap-3 mt-0.5">
                          ...
                          <span className="font-body text-[0.65rem] text-cream-soft">📍 {appt.location}</span>
                        </div>
                      </div>
          ...
          {Object.keys(grouped).length === 0 && (
            <div className="text-center py-16 font-body text-cream-soft">No appointments found</div>
          )}
        </div>
      )}
          ...
          <motion.div className="fixed inset-0 z-[300] bg-charcoal/85 backdrop-blur-sm flex items-center justify-center p-4"
            ...
            <motion.div className="w-full max-w-[480px] bg-charcoal-mid border border-gold/15"
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-gold/10">
                ...
                <button onClick={() => setSelected(null)} className="text-cream-soft hover:text-gold"><X size={16} /></button>
              </div>
              <div className="p-6 space-y-4">
                {[[User, selected.client_name],[Mail, selected.client_email],[Phone, selected.client_phone],[Calendar, `${selected.preferred_date} at ${selected.preferred_time?.slice(0,5)}`],[MapPin, selected.location],[Clock, `${selected.duration_mins} minutes`]].filter(([,v])=>v).map(([Icon,val],i)=>(
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <span className="font-body text-[0.85rem] text-cream-soft">{val}</span>
                  </div>
                ))}
                {selected.notes && <p className="font-body text-[0.82rem] text-cream-soft bg-charcoal border border-gold/8 p-4 mt-2">{selected.notes}</p>}
